...
function pickAssignee(requester, approvalType, env) {
...
module.exports = async (req, res) => {
  try {
    ...
    const assigneeMaybe = pickAssignee(data?.requester, data?.approvalType, process.env);
    const stateId = (process.env.LINEAR_WORKFLOW_STATE_ID || '').trim();
    const projectId = (process.env.LINEAR_PROJECT_ID || '').trim();

    const inputAny = { teamId, title, description };
    ...
    const issue = resp?.data?.data?.issueCreate?.issue || null;
    if (!issue) {
